...
class GameProcessor {
  ...
  static MOVE_FORWARD_CRITERIA = 4;
  ...
  playRounds(names, count) {
    ...
    for (let i = 0; i < count; i++) {
      this.cars.map((car) => {
        ...
        if (randomNumber >= GameProcessor.MOVE_FORWARD_CRITERIA) {
          car.moveForward();
        }
      });
...
const gameReader = new GameProcessor();

async function play() {
  const names = await gameReader.readCarNames();

  const count = await gameReader.readTryCount();

  gameReader.playRounds(names, count);
  gameReader.displayWinner();
}
